...
def run():
    print("Clearing old scores and features...")
    ...
    print("Creating features per category...")
    for c_name, f_names in category_features.items():
        try:
            ...
            print(f"Category {c_name} not found, skipping feature creation.")


    # Specific overrides by product name substring
    # If a product has a name containing a key below, we use these values.
    # We match them to the category's feature names.
    product_scores = {
    ...
    for p in products:
        ...
        # Apply score or a default of 80 if not defined
        for f in c_features:
    ...
    print(f"Created {created} scores for {products.count()} products.")

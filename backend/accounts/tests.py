...
class AccountsTests(TestCase):
    ...
    def test_credential_submit(self):
        response = self.client.post("/api/auth/test-register/submit/", {"email": "submit@example.com", "password": "abc"})
    ...
    def test_credential_verify_success(self):
        ...
        response = self.client.post("/api/auth/test-register/verify/", {"id": cred.id, "code": "123456"})
        self.assertEqual(response.status_code, 201)
    ...
    def test_credential_verify_fail(self):
        ...
        response = self.client.post("/api/auth/test-register/verify/", {"id": cred.id, "code": "000000"})
